    ...
    document.addEventListener(
        ...
        function (event) {
            ...
            if (target.matches("#submit-create-race")) {
                ...
                // check if the user has selected a racer and a track before
                if (!store.player_id || !store.track_id) {
                    alert(
                        "Please select a racer and a track before starting the game"
                    );
                    return;
                }
                // start race
...
async function handleCreateRace() {
    ...
    try {
        ...
        console.log("problem found while creating the race: ", err);
    }
}
    ...
    return new Promise((resolve) => {
        const raceInterval = setInterval(async () => {
            try {
                ...
                console.log(
                    `There is an Error in the interval of running the race:, ${err}`
                );
                clearInterval(raceInterval);
                resolve();
            }
...
async function runCountdown() {
    try {
        ...
        console.log(`there was an error in the countdown function: ${error}`);
    }
}
...
function renderRacerCard(racer) {
    ...
    return `<h4 class="card racer" id="${id}">${driver_name} <br/> <br/> Top Speed : ${top_speed} <br/> Acceleration: ${acceleration} <br/> Handling: ${handling} </h4>`;
}
...
function resultsView(positions) {
    ...
    return `
		...
		<main>
			...
			${results.join("")}
			<a class="newButton" href="/race">Start a new race</a>
		</main>
	`;
}

function raceProgress(positions) {
    userPlayer = positions.find((e) => e.id === parseInt(store.player_id));
    userPlayer.driver_name += " (you)";
...
async function getTracks() {
    ...
    try {
        ...
        console.log(`There was an error in the getTracks function: ${err}`);
    }
}
...
async function getRacers() {
    ...
    try {
        ...
        console.log(`There was an error in the getRacers function: ${err}`);
    }
}
...
async function getRace(id) {
    ...
    try {
        ...
        console.log(`There was an error in the getRace function: ${err}`);
    }
}
